...
import (
	...
	"github.com/shardit-io/ordo/http/crudgin"
...
func TestGinHTTPPagination(t *testing.T) {
	b := blogs(t)[0]
	seedBlog(t, b)
	app := newGinApp(t, b)

	r := ginCall(t, app, http.MethodGet, "/articles?limit=2&page=1&sort=title", nil)
	var p page
	r.decode(t, &p)
	...
	r = ginCall(t, app, http.MethodGet, "/articles?limit=2&page=2&sort=title", nil)
	p = page{}
	r.decode(t, &p)
...
func TestGinHTTPCount(t *testing.T) {
	b := blogs(t)[0]
	seedBlog(t, b)
	app := newGinApp(t, b)
	...
	ginCall(t, app, http.MethodGet, "/articles/count?f=views:gte:50", nil).decode(t, &out)
	if out.Count != 2 {
		t.Fatalf("count = %d", out.Count)
	}
	ginRaw(t, app, http.MethodPost, "/articles/count", `{"filter":{"tags.slug":"rust"}}`).decode(t, &out)
	if out.Count != 2 {
		t.Fatalf("count = %d", out.Count)
	}
}

func TestGinHTTPCrudLifecycle(t *testing.T) {
	for _, b := range blogs(t) {
		t.Run(b.name, func(t *testing.T) {
			...
			r = ginRaw(t, app, http.MethodPatch, "/articles/"+itoa64(created.ID), `{"PublishedAt": null}`)
			r.decode(t, &patched)
			...
			r = ginCall(t, app, http.MethodGet, "/articles/"+itoa64(created.ID), nil)
...
func TestGinHTTPBulkDelete(t *testing.T) {
	b := blogs(t)[0]
	...
	ginCall(t, app, http.MethodPost, "/articles/bulk-delete",
	...
	ginCall(t, app, http.MethodGet, "/articles/count", nil).decode(t, &count)
	if count.Count != 1 {
...
func TestGinHTTPRejections(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			r := ginRaw(t, app, tc.method, tc.target, tc.body)
			if r.status != tc.want {
				t.Fatalf("status %d (want %d): %s", r.status, tc.want, r.body)
			}
			var body crudgin.ErrorBody
			r.decode(t, &body)
			if body.Error == "" {
...
func TestGinHTTPWorksWithoutExtraDeclarations(t *testing.T) {
	...
	r := ginCall(t, app, http.MethodGet, "/comments?preload=author&f=approved:eq:true&sort=body", nil)
	if r.status != http.StatusOK {
		t.Fatalf("status %d: %s", r.status, r.body)
	}
	var out struct {
