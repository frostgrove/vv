...
`RegisterPGXPoolStats` and one `NewRuntimeMetrics` handle per provider. Start that
handle exactly once during application assembly; repeated `Start` calls on the
same handle are idempotent. No recipe changes an OTel
...
| Go runtime | unexpected resource/scope identity or new labels after dependency upgrades | one application-owned start handle per provider | exact runtime scope; only closed `go.memory.type` where applicable | fixed eight-instrument roster; all other attributes dropped | `runtime_metrics_test.go`, `db_runtime_projection_test.go` |
