...
version: ## Point every submodule at a library version: make version V=v0.1.0
	...
	@for m in $(MODULES); do \
		test "$$m" = "." && continue; \
		(cd $$m && $(GO) mod edit -require=github.com/shardit-io/go-rx-crud@$(V)) || exit 1; \
		echo "$$m -> $(V)"; \
	done
...
release: test ## Tag a release: make release V=v0.1.0
	...
	@echo "consumers:"
	@echo "  go get github.com/shardit-io/go-rx-crud@$(V)"
	@for m in $(MODULES); do \
		test "$$m" = "." && continue; \
		echo "  go get github.com/shardit-io/go-rx-crud/$$m@$(V)"; \
	done
