...
var (
	...
	Docs  = basic.Define[Doc, string, DocUpdate]("docs")
)
...
func TestBadDeclarationsPanicEarly(t *testing.T) {
	...
	if _, err := basic.TryDefine[NoKey, int64, struct{}]("x"); err == nil {
		t.Error("a model without a primary key should not define")
	}
	if _, err := basic.TryDefine[User, string, UserUpdate]("users"); err == nil {
		t.Error("an ID type that disagrees with the primary key should not define")
	}
	if _, err := basic.TryDefine[User, int64, Mismatch]("users"); err == nil {
		t.Error("an update DTO with the wrong field type should not define")
	}
	if _, err := basic.TryDefine[User, int64, Unknown]("users"); err == nil {
		t.Error("an update DTO naming a field the model lacks should not define")
	}
	if _, err := basic.TryDefine[User, int64, Immutable]("users"); err == nil {
