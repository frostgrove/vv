...
func TestBadDeclarationsAreRefusedAndSayWhy(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			func() error { _, err := basic.TryDefine[int, int, struct{}]("counters"); return err },
			func() { basic.Define[int, int, struct{}]("counters") },
			"model must be a struct",
		},
		{
			"a model with no primary key",
			func() error { _, err := basic.TryDefine[NoKey, int64, struct{}]("nokeys"); return err },
			func() { basic.Define[NoKey, int64, struct{}]("nokeys") },
			"no primary key",
		},
		{
			"an ID type that is not the primary key's",
			func() error { _, err := basic.TryDefine[User, string, UserUpdate]("users"); return err },
			func() { basic.Define[User, string, UserUpdate]("users") },
			"repository ID type is string but the primary key is int64",
		},
		{
			"an update DTO that is not a struct",
			func() error { _, err := basic.TryDefine[User, int64, string]("users"); return err },
			func() { basic.Define[User, int64, string]("users") },
			"update DTO must be a struct",
		},
		{
			"a DTO field the model does not have",
			func() error { _, err := basic.TryDefine[User, int64, Unknown]("users"); return err },
			func() { basic.Define[User, int64, Unknown]("users") },
			"no field Nope on model User",
		},
		{
			"a DTO field of the wrong type",
			func() error { _, err := basic.TryDefine[User, int64, WrongType]("users"); return err },
			func() { basic.Define[User, int64, WrongType]("users") },
			"type mismatch: DTO carries int, model field Name is string",
		},
		{
			"a DTO field the model froze",
			func() error { _, err := basic.TryDefine[User, int64, Frozen]("users"); return err },
			func() { basic.Define[User, int64, Frozen]("users") },
			"model field TenantID is `immutable`",
		},
		{
			"a DTO that would rewrite the primary key",
			func() error { _, err := basic.TryDefine[User, int64, Key]("users"); return err },
			func() { basic.Define[User, int64, Key]("users") },
			"the primary key cannot be updated",
		},
		{
			"a DTO field the database owns",
			func() error { _, err := basic.TryDefine[User, int64, Computed]("users"); return err },
			func() { basic.Define[User, int64, Computed]("users") },
...
func TestAnEmptyTableNameBecomesThePluralOfTheModel(t *testing.T) {
	...
	if _, err := basic.New[Story, int64, struct{}](rec, "").GetAll(context.Background()); err != nil {
		t.Fatal(err)
	}
...
func TestMaxLimitCapsEvenTheDefaultLimit(t *testing.T) {
	strict := basic.Define[User, int64, UserUpdate]("users", basic.DefaultLimit(50), basic.MaxLimit(10))
...
func TestANonPositiveDefaultLimitFallsBackToThePackageDefault(t *testing.T) {
	for _, n := range []int{0, -5} {
		t.Run(strconv.Itoa(n), func(t *testing.T) {
			rec := crudtest.Postgres().Push(crudtest.Rows())
			repo := basic.Define[User, int64, UserUpdate]("users", basic.DefaultLimit(n)).Bind(rec)
			if _, err := repo.Get(context.Background()); err != nil {
				t.Fatal(err)
			}
			want := "LIMIT " + strconv.Itoa(basic.DefaultPageSize)
...
func TestAnUnknownDefaultSortIsRefusedBeforeTheQueryIsSent(t *testing.T) {
	rec := crudtest.Postgres().Push(crudtest.Rows())
	repo := basic.Define[User, int64, UserUpdate]("users", basic.DefaultSort(crud.Desc("Nope"))).Bind(rec)
...
func TestPreloadDepthCapsAPathAndZeroMeansUnset(t *testing.T) {
	...
	t.Run("one hop is allowed at a depth of one", func(t *testing.T) {
		...
		repo := basic.Define[Author, int64, struct{}]("authors", basic.PreloadDepth(1)).Bind(rec)
	...
	t.Run("zero leaves the default depth in place", func(t *testing.T) {
		...
		repo := basic.Define[Author, int64, struct{}]("authors", basic.PreloadDepth(0)).Bind(rec)
...
var scopedUsers = basic.Define[User, int64, UserUpdate]("users", basic.Scope(crud.Eq("TenantID", int64(1))))
