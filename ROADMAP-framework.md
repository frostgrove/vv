...
**Scope:** what `github.com/shardit-io/ordo` is, how its modules are drawn, what
...
The repository was `go-rx-crud`, is `github.com/shardit-io/ordo` on disk today, and
is becoming **`github.com/shardit-io/ordo`** — Latin for order, rank, and an
...
- **D-033's own proof command prints 17 lines on a clean tree.** It ends in
  ...
  ```
  go list -deps -f '{{if not .Standard}}{{.ImportPath}}{{end}}' ./... | grep -v '^github.com/shardit-io/ordo'
  ```
...
**Rename 2: `rx` → `ordo`. Decided, not yet executed.** `github.com/shardit-io/ordo`.
