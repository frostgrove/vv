// The Gin sign-in surface is its own module so a consumer on Fiber or net/http
// never takes Gin as a dependency. See D-033.
//
// It requires the access module and Gin, and nothing else. Serving these
// routes and serving a CRUD resource are two things a consumer chooses
// separately, so this does not require crudgin — see D-051.
module github.com/frostgrove/vv/auth/access/http/accessgin
