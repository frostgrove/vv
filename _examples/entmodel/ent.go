	...
	return func(s *sql.Selector) {
		for _, f := range fields {
			if err := checkColumn(s.TableName(), f); err != nil {
				s.AddError(&ValidationError{Name: f, err: fmt.Errorf("entmodel: %w", err)})
			}
...
//	Aggregate(entmodel.As(entmodel.Sum(field1), "sum_field1"), (entmodel.As(entmodel.Sum(field2), "sum_field2")).
//	Scan(ctx, &v)
...
func Max(field string) AggregateFunc {
	return func(s *sql.Selector) string {
		if err := checkColumn(s.TableName(), field); err != nil {
			s.AddError(&ValidationError{Name: field, err: fmt.Errorf("entmodel: %w", err)})
			return ""
		}
