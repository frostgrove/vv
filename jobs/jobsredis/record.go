...
func encodeRecord(record jobs.DeliveryRecord) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("jobsredis: encode delivery record: %w", err)
	}
	return encoded, nil
}
...
func decodeRecord(encoded []byte) (jobs.DeliveryRecord, error) {
	...
	if err := json.Unmarshal(encoded, &dto); err != nil {
		return jobs.DeliveryRecord{}, fmt.Errorf("jobsredis: decode delivery record: %w", err)
	}
	if dto.Version != 1 {
		return jobs.DeliveryRecord{}, fmt.Errorf("jobsredis: unsupported delivery record version %d", dto.Version)
	}
...
func decodeGenesis(dto genesisDTO) (jobs.InvocationGenesisRecord, error) {
	...
	return jobs.InvocationGenesisRecord{
		...
		Policy:       policy,
		Context:      decodeContext(dto.Context),
	}, nil
}
...
func decodeOutcome(dto outcomeDTO) (jobs.InvocationOutcome, error) {
	...
	default:
		return jobs.InvocationOutcome{}, fmt.Errorf("jobsredis: invalid invocation outcome kind %d", dto.Kind)
	}
}
...
func recordFromInvocation(invocation jobs.Invocation, previous jobs.DeliveryRecord) (jobs.DeliveryRecord, error) {
	payload, err := jobs.NewEncodedPayload(previous.Payload.Codec, previous.Payload.Version, previous.Payload.Data)
	if err != nil {
		return jobs.DeliveryRecord{}, err
	}
	return jobs.NewDeliveryRecord(invocation, payload, previous.WireDigest, previous.PayloadDigest)
}
