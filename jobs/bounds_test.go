...
func TestJobBoundsAreExact(t *testing.T) {
	...
	durations := map[string]struct {
		...
		"minimum operation timeout": {MinimumOperationTimeout, 10 * time.Millisecond},
		"operation timeout":         {DefaultOperationTimeout, 10 * time.Second},
		"maximum operation timeout": {MaximumOperationTimeout, time.Minute},
