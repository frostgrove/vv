...
func TestABulkMutationReachesTheDatabaseNarrowed(t *testing.T) {
	...
	if _, err := invoices.DeleteAll(ctx); err != nil {
		t.Fatal(err)
	}
	mutating := ""
	for _, statement := range recorder.SQL() {
		if strings.HasPrefix(statement, "DELETE") || strings.HasPrefix(statement, "UPDATE") {
			mutating = statement
		}
	}
	if mutating == "" {
	...
	if !strings.Contains(clauseOf(mutating), `"tenant_id" = $`) {
		t.Fatalf("the delete ran across every tenant: %s", mutating)
	}
}
...
func TestAnUnscopedBulkMutationIsRefusedRatherThanWidened(t *testing.T) {
	...
	if _, err := invoices.UpdateAll(ctx, InvoiceUpdate{Number: &number}); err != nil {
		t.Fatalf("a bulk update inside one tenant was refused: %v", err)
	}
	for _, statement := range recorder.SQL() {
		if !strings.Contains(statement, "tenant_id") {
			t.Fatalf("a bulk statement ran across every tenant: %s", statement)
		}
	}
}
...
func TestARowOwnedThroughARelationIsNarrowedByThatRelation(t *testing.T) {
	...
	statement := recorder.Last().SQL
	if !strings.Contains(statement, "tenant_id") || !strings.Contains(statement, "invoices") {
		t.Fatalf("the read does not reach the owner through the relation: %s", statement)
	}
...
func TestAPreloadOfADeclaredRelationCarriesTheTenant(t *testing.T) {
	...
	if len(statements) < 2 {
		t.Fatalf("the preload issued no second statement: %v", statements)
	}
	if !strings.Contains(clauseOf(statements[1]), "tenant_id") {
		t.Fatalf("the preload read the child table raw: %s", statements[1])
	}
