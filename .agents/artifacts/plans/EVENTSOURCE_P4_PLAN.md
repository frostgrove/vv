...
| `topology` is added to the `undecorated` exemption list while its defects stand | the same test — *"the exemption is stale and the list only ever shrinks"*. The list shrank to nothing on 2026-09-13 and was removed with it: `durability`, its last member, gained the `kept` defect row (EVENTSOURCE_DETECTION GAP-2), so this mutation has no target any more |
