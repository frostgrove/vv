...
func (h *Handler[M, ID, U]) count(c fiber.Ctx, req *query.Request) error {
	// Paging, sorting and preloads mean nothing to a count.
...
func (h *Handler[M, ID, U]) GetByID(c fiber.Ctx) error {
	...
	// Only the shaping options make sense for a single entity.
...
func (h *Handler[M, ID, U]) BulkDelete(c fiber.Ctx) error {
	...
	if h.opt.maxBulk > 0 && len(req.IDs) > h.opt.maxBulk {
		return h.fail(c, badRequestf("at most %d ids per request", h.opt.maxBulk))
	}
...
// sanitize clears what a client is not allowed to choose on create.
func (h *Handler[M, ID, U]) sanitize(m *M) error {
	if h.meta.PK.Auto && !h.opt.allowClientID {
		if err := h.meta.SetID(m, reflect.Zero(h.meta.PK.Type).Interface()); err != nil {
			return err
		}
	}
	return h.clearGenerated(m)
}
...
func (h *Handler[M, ID, U]) clearGenerated(m *M) error {
	...
	for _, f := range h.meta.Fields {
		if f.Generated {
			reflect.NewAt(f.Type, unsafeAdd(base, f.Offset)).Elem().SetZero()
		}
	}
	return nil
}
