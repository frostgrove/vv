...
// do sends one request through the mounted app. A non-empty body is sent as
// JSON, which is the only body type Bind().Body accepts.
func do(t *testing.T, app *fiber.App, method, target, body string) response {
	t.Helper()
	var rdr io.Reader
	if body != "" {
...
func ok(t *testing.T, app *fiber.App, method, target, body string, want int) response {
	t.Helper()
	r := do(t, app, method, target, body)
	if r.status != want {
