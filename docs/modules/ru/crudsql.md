...
Он предоставляет `*sql.DB` и `crud.Source` поверх него, проверяет соединение
**на старте** и закрывает пул при остановке. `sql.Open` валидирует DSN и никуда
не подключается, поэтому без этой проверки неверный хост, неверный пароль и
незапущенная база одинаково выглядят как здоровый старт.

Проверка — хук старта, а не строка в конструкторе: конструкторы исполняются
внутри `fx.New`, куда `fx.StartTimeout` не достаёт, и недоступный сервер вешал
бы сборку графа — процесс не стартовал бы и не падал. Одно из конструктора уйти
не может: `crud.Source` — зависимость каждого репозитория в графе, а его
построение читает схему. Это чтение вместо переноса ограничено сроком — им
служит `Pool.ConnectTimeout`, если конфигурация его называет, и
`DefaultSchemaTimeout` (15 с) иначе.
