...
- [[D-107]] ресурс объявляет права, которые проверяет его гейт · [[FL-024]] гейт на старте · [[FL-020]] откуда берётся значение политики
...
| `Policy` | `RequiredFor(crud.Action) ([]auth.Permission, bool)` — то, что спрашивает `GuardedBy`; `security.Policy` это умеет |
...
политика не объявляет: маршруты названы, декларация не возвращается. Действие,
объявленное без прав, становится `authhttp.Authenticated` — именно это значит
`RequirePermission()` без аргументов. `Guarded` сводит create и update к `write`,
а оба удаления к `del`; `GuardedBy` их не смешивает.
...
**Пути берутся из таблицы, а права — из гейта.** Декларация, выписанная руками,
совпадает с роутером ровно до того, как кто-то добавит маршрут; выведенная из
роутера — совпадает всегда, в том числе когда обе неверны, поэтому пути сверяются
с реальной таблицей маршрутов, а не пишутся дважды ([[D-073]]). Права раньше были
той половиной, которую стоило утверждать дважды: ничто не могло сравнить
декларацию маршрута с проверкой, которая идёт за ним. Теперь `security.Policy`
хранит требование данными, и `GuardedBy` читает тот самый список, который
проверяется ([[D-107]]).

У маршрута, который не является CRUD-ресурсом, таблицы нет: он закрыт внутри тела
самого юзкейса, и его декларацию выводит из этого guard'а `vv generate routes`
([[D-109]], [cmd/vv](vv-cli.md)).
