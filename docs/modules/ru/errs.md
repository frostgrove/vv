...
`Add` возвращает `errs.ErrCodeRedeclared`, если код уже объявлен с другим kind.
Пустые, слишком длинные и невалидные UTF-8-коды, неизвестные kind, а также
некорректные или слишком длинные шаблоны по умолчанию отклоняются. В остальном
код непрозрачен: приложение не обязано принимать одно соглашение об именах.
`Add`, `KindOf`, `MessageFor` и метод `MessageSource` `Message` можно вызывать
конкурентно. Нулевое значение работает; `nil *Codes` читается как пустой, а
попытка добавить в него код возвращает ошибку, а не паникует. `Message`
подставляет в шаблон разрешённые параметры нарушения и отказывается отвечать,
если параметра не хватает; поэтому `*Codes` сам по себе служит безопасным
последним `MessageSource`.
...
Каждый файл должен быть корректным UTF-8 и содержать один плоский JSON-объект
со строковыми значениями. Повторяющиеся члены объекта считаются ошибкой, в том
числе две экранированные записи одного ключа. Ключи, имена локалей и имена
подстановок — ограниченные непрозрачные UTF-8-строки. Это намеренно: настоящие
элементы публичного пути и прикладные коды могут содержать точки, пробелы и
другую пунктуацию. Ключ и имя подстановки не могут быть пустыми, а имя
подстановки не может содержать скобки. В шаблонах используются подстановки
`{name}`; непарные скобки отклоняются в `Add` и `Load`.
...
записывающий весь путь целиком, не рассматривается. Загрузчик не может
отклонить такую последовательность байтов: путь с первым элементом, буквально
равным `order.items`, рассматривает тот же ключ. Определяйте достижимость по
объявленному отображению публичного пути, а не по числу точек.

`Messages.Add` и `Messages.Load` безопасны, пока запросы вызывают `Message`,
`MessageWithLocale`, `Locales` и `Missing`. `Load` читает и проверяет все JSON-файлы до одной
атомарной фиксации. Ошибка чтения, проверки или конфликт объявления оставляет
существующий каталог неизменным; `Add`, завершившийся во время подготовки
файлов, сливается с ними при фиксации и не теряется. `Locales()` возвращает
отсортированный список.

Имя локали — ограниченная по длине непрозрачная строка, а не канонизация BCP
47. Сопоставление чувствительно к регистру. И `en-GB`, и `en_GB` сначала
проваливаются в `en`, затем поиск сообщения — в `default.json` и наконец в
значение кода по умолчанию. Если параметра подстановки нет, шаблон
отказывается отвечать и поиск переходит к следующей ступени.

`Missing(locale)` намеренно строже поиска сообщения: он учитывает названную
локаль и её базовую локаль, но `default.json` не скрывает отсутствующий перевод
для другой локали. Подключите это к тесту, и наполовину переведённый каталог
завалит сборку. `Missing("")` проверяет сам `default.json`.
...
Этот плоский каталог не реализует правила множественного числа. Продукт,
которому нужен выбор по CLDR или ICU, предоставляет другой `MessageSource`;
остальная цепочка обработки ошибок не меняется.

Полный путь предоставляет optional-модуль [i18n](i18n.md), не меняя `errs`:
`Snapshot.ErrorMessages` проверяет immutable mappings лестницы, разрешённые
параметры нарушения и сообщения названий полей, затем реализует
`LocalizedMessageSource`. Если mapping, bind или render не удался, адаптер
отказывается отвечать, сохраняя прежний fallback публичного сообщения/кода и
обезличивание Internal.

---

## SPI — шесть интерфейсов, которые реализует третья сторона
...
| `MessageSource` | `Message(ctx, Violation, locale) (string, bool)` | `errs.Messages`, `errs.Codes` или вашей библиотекой i18n |
| `LocalizedMessageSource` | `MessageWithLocale(ctx, Violation, locale) (text, actualLocale string, ok bool)` плюс `MessageSource` | `errs.Messages` или библиотекой i18n, способной доказать, из какой локали взят шаблон |
