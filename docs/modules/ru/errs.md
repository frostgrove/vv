...
## Вид, который может поднять только роутер

`KindMethodNotAllowed` и `CodeMethodNotAllowed` — это обслуживаемый путь,
отвеченный глаголом, которого у него нет.

Это отдельный вид, а не код внутри `KindBadRequest`, по той же причине, что и
`KindTooLarge`: статус — это всё, по чему действует клиент. 405 говорит «спросите
у этого пути что-нибудь другое», а 404 — «этого пути здесь нет», и клиент,
повторяющий второй, повторял бы вечно. Транспорты отображают вид и никогда код,
так что один код этого не унёс бы.

Это единственный вид, который не может поднять ничего ниже транспорта:
репозиторий не знает, какие глаголы есть у пути. Его конструктор —
`errs.MethodNotAllowed()`; `porthttp.Routed(status)` — то, что привязка вызывает
для собственного отказа своего роутера.
