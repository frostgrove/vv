...
**Ссылка не может пережить подписавшие её учётные данные.** Клиент на STS, IRSA
или роли инстанса держит сессию в пределах часа, а presigned URL перестаёт
работать в момент её истечения — поэтому запрос более длинного TTL получает
именованный отказ вместо ссылки со сроком, который object store не соблюдёт.
У клиента со статическими ключами срока нет, и его это не касается.

**`Delete` удаляет только текущую версию.** На versioned bucket это пишет delete
marker, а не стирает что-либо; `Info.Version` — дескриптор, который не принимает
ни одна операция, кроме условного удаления. Стирание на versioned bucket — это
lifecycle-политика оператора, а не задача адаптера.

**Stage отклоняется, если стандартное продвижение его никогда не разместит.**
Полезная нагрузка выше `MaxCreateOnlySize` отвергается на `Stage` — по заявленному
размеру до загрузки и по реальному после — вместо того чтобы быть принятой,
оплаченной и удержанной на весь TTL и упасть только на `Promote`.

**`Config.StageClaimTTL` ограничивает прерывание, а не stage.** Один `Stage`,
`Promote` или `Abort` держит claim столько (по умолчанию пять минут); после
этого stage может забрать другой процесс. Уборка, сломавшая такой claim,
сообщает `CleanupResult.More`.

**Интеграционных сценариев в этом адаптере нет.** Развёртывание, заявляющее
совместимость с MinIO, пишет их само — конкурентный условный одиночный `PUT`,
`Put` с неверным `IfMatch`, multipart stage с последующим `Promote` в режиме
`Replace`, presigned `GET` в том числе после истечения сессии подписи, и отказ
read-quorum во время условного `PUT`.
