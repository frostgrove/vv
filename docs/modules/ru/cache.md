...
`BatchReadCapability` может прочитать всё одной операцией. Без этой возможности
он читает по одному адресу, и адрес, на который backend не ответил, — это промах
**по этому адресу**: уже прочитанные записи сохраняются, поэтому один сбойный
ключ не отправляет весь пакет обратно в origin.

`Put` и `Forget` ограждены от параллельных loader'ов: старый loader не
перезапишет более новую мутацию. **Это ограждение локально для процесса**: это
счётчик поколения адреса внутри процесса, поэтому он упорядочивает загрузку и
мутацию, выпущенные одним процессом, и ничего не говорит про другую реплику,
пишущую тот же ключ в тот же момент. На общем backend между процессами по-прежнему
побеждает последняя запись. Межпроцессное ограждение пришло бы от backend с
`CompareAndSwapCapability`, и ядро его пока не использует.
...
Короткая запись о наблюдаемом поведении: каждое из этого было обещанием, которого
код не выполнял.

**Ротация `ValueSchema` — это промах, а не повреждение.** К моменту сравнения
схемы хеш конверта уже проверен, значит байты целы: они записаны предыдущей
схемой, что развёртывание делает намеренно при изменении кэшируемого типа. Чтение
их как повреждения приводило к тому, что рутинная ротация проваливала каждый
`Resolve` при `RefuseCorrupt`, loader не запускался, и запись никто не вытеснял.

**Ошибка записи в кэш никогда не проваливает `Resolve`, а `Put` никогда её не
скрывает.** Это была одна ручка `WriteFailure`, направленная в противоположные
стороны: при `Propagate` backend, не принявший значение, уничтожал значение,
которое loader уже получил, а при `Ignore` явный `Put` возвращал `nil`, ничего не
сохранив. На каждом пути есть один правильный ответ, поэтому ручки больше нет.

**`MaxFlights` ограничивает число различных одновременных загрузок по всему
кэшу**, и поставляемые профили теперь допускают восемь, а не одну. Слияние по
адресу безусловно и отдельно, поэтому значение 1 никогда не означало «одна
загрузка на ключ» — оно означало, что второму промахнувшемуся ключу отказывают с
`ErrSaturated`. Transient-бюджет, который объявляет каждый профиль, теперь тот,
что допускает его собственное число flights, а
`PolicyDescription.ConcurrentResolves` сообщает, сколько сквозных чтений бюджет
допускает.

**`Namespace.Digest()` экспортирован**, чтобы backend вне этого пакета мог
сопоставить namespace с адресами, записанными под ним, — это нужно
`TagInvalidator` и было невозможно.

**`cachefx.AsObserver` вносит в группу.** Два модуля, желающие наблюдать за кэшем,
