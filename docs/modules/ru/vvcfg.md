...
Ошибки file/environment decoder проходят display-safe границу: некоторые
parser-ы дословно цитируют неразобранное значение (реальный пример —
незакрытый пароль в `.env`), поэтому `Load` возвращает фиксированный текст и не
эхоит diagnostic. Исходный cause остаётся доступен через `errors.Is` и
`errors.As`.
Ошибки typed `EnvironmentApplier` остаются видимыми: это принадлежащая
приложению actionable validation. Реализация должна называть поле или
переменную, но никогда не выводить её значение.
