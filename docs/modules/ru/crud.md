...
SQL здесь никто не выполняет — этим занимается `repo/basic` — и здесь не знают,
что такое транспорт.
...
вручную, если это нужно; `basic.Define` делает это за вас и валидирует сразу.
...
Целевые таблицы разрешаются по регистрации в `basic.Define`, затем по методу
...
- [basic](basic.md) — репозиторий, превращающий всё это в SQL
