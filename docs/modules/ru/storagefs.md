...
## Что здесь означает `Info`

`ETag` выводится из размера объекта и наносекундной метки записи, которая его
породила. Именно его сравнивает `IfMatch`, и он меняется при каждой записи.

`ModifiedAt` — момент, когда объект стал **виден**, а не когда были записаны его
байты. Для продвинутого (`Promote`) объекта это существенно: его заголовок был
записан при `Stage`, что могло быть днями раньше, и такой ответ разошёлся бы с
`storageminio` про один и тот же объект. `Promote` перештамповывает заголовок
непосредственно перед rename, который его публикует.

`Config.StageClaimTTL` (по умолчанию пять минут) — сколько один `Promote` или
`Abort` держит stage, прежде чем его сможет забрать другой. Он ограничивает
*прерывание*: без него процесс, убитый посреди promote, заклинивал `StageID` до
собственного TTL стейджа — днями, — а `Promote` и `Abort` всё это время отвечали
`ErrConflict`.

`Delete` также удаляет опустевшие каталоги, созданные ключом, вплоть до корня
namespace, по мере возможности. Каждый ключ создаёт по каталогу на сегмент, и
больше их никто не освобождал: `List` нет, а `CleanupExpired` обходит только
staging.

Обработчик ссылок отвечает на запрос одного диапазона байт кодом `206` с
`Content-Range` и объявляет `Accept-Ranges: bytes`; запрос нескольких диапазонов
отдаётся целиком, с `200` и без `Content-Range`, чтобы ни одному клиенту не
сообщили, что он получил не то, что получил.
