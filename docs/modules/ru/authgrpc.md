...
A -> B -> A отклоняется: interceptor не угадывает, B был step-up или downgrade.
Оба конструктора вызывают `Guard.Validate`, поэтому nil и `new(auth.Guard)`
паникуют до запуска сервера ([[D-076]]).
