...
## Мультитенантность — один арендатор, проверенный, до самого низа

| Модуль | Импорт | Что это |
|---|---|---|
| [tenancy](tenancy.md) | `vv/tenancy` (+ `tenancyrow`, `tenancydb`, `tenancyjobs`, `tenancystorage`, `tenancycache`) | Опционально. Проверенный scope в ядре, которое не тянет ни одного шва, и по пакету на шов поверх него: строки, базы, долговременная работа, объекты, кэш |
...
## Event sourcing — история вместо текущей строки

| Модуль | Импорт | Что это |
|---|---|---|
| [event](event.md) | `vv/event` | Опционально. Объявленные факты, цепочка читателей на каждую форму полезной нагрузки, добавление по ожидаемой версии, полное воспроизведение, ограниченный обход журнала и словарь отказов из шести классов — поверх любого хранилища |
| [eventmemory](eventmemory.md) | `vv/event/eventmemory` | Полноценное хранилище в памяти, вместе с транзакциями. Не заглушка |
| [eventpg](eventpg.md) | `vv/event/eventpg` | **Модуль** — хранилище на PostgreSQL: добавление одним оператором, трёхуровневая проверка схемы, неизменяемая история и обход журнала по осевшему водяному знаку |
...
| [eventtest](eventtest.md) | `vv/event/eventtest` | Пять наборов проверок соответствия: контракт хранилища, контракт контрольных точек и три интерфейса, которые реализует приложение, — `Ledger`, `Generations`, `Park` — плюс три прокси, которые оно прогоняет над своим объявлением |
| [receipt](receipt.md) | `vv/event/receipt` | Долговечная квитанция операции рядом с добавлением, в собственной транзакции вызывающего и собственной таблице приложения: заявка до решения, отпечаток по байтам, три вердикта и четыре положения — и отсутствующая строка, которая никогда не является откатом |
...
| Модуль | Импорт | Что это |
|---|---|---|
| [i18n](i18n.md) | `github.com/frostgrove/vv/i18n` | **Модуль** — неизменяемые проверенные MF2-каталоги, точные типизированные значения, locale/formatting policy, rich/bidi output, формулировки ошибок, консервативный type-aware offline authoring и ограниченная атомарная публикация |

## Observability — трассировка и метрики

| Модуль | Импорт | Что это |
|---|---|---|
| [otel](otel.md) | `vv/otel` | **Модуль** — декораторы команд сервиса и хранилища OpenTelemetry, наблюдатели терминальных событий кэша, ограниченная схема и гистограмма длительности |
...
| [lock](lock.md) | `vv/vvdb/lock` | Advisory-блокировки на выбранном ключе: разделяемые и эксклюзивные guard, один детерминированный порядок, повтор того, что чинится второй попыткой, и типизированный отказ движку, который так не умеет. Рядом `locksql` для драйвера с `database/sql` |
