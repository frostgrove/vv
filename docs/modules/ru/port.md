...
## Правила, которые биндингу не принадлежат

Пять настроек хендлера не говорят ничего о транспорте: что клиент может
запросить, что он не может выбрать сам и сколько может прийти за раз. Они лежат
здесь один раз, и каждый биндинг их встраивает:
...
которую биндинг поднимает, если такую опцию передали в `Serving`, где сервис уже
собран ([[D-021]]).

*Конструкторы* опций остаются в биндингах: `Option` у каждого свой, с тремя
параметрами ([[D-045]]), и общий конструктор не смог бы вернуть его. Общее — это
состояние и два метода над ним, то есть ровно та половина, которая раньше
копировалась четыре раза и из-за которой `MaxBody` пришлось бы писать четырежды,
чтобы он вообще появился.

`Rules.BulkCap()` отвечает, сколько id принимает bulk delete: поле, если оно
задано, и `DefaultMaxBulk` (1024) иначе. Метод, а не поле со значением по
умолчанию, чтобы четыре транспорта не могли разойтись в том, что означает
незаданный `MaxBulk` — а именно так они и сошлись на том, что он означает
отсутствие лимита ([[D-060]]).

`MaxBody` учитывают только три HTTP-биндинга; gRPC ограничивает сообщение на
сервере своим `MaxRecvMsgSize`, до того как запустится хендлер.

`crudhttp.Rules` — алиас этого типа, под именем, которое ищет HTTP-биндинг.

## Куда уходят собственные строки библиотеки

```go
port.Logger(ctx)                 // логгер из контекста или slog.Default()
ctx = port.WithLogger(ctx, log)  // положить свой
```

Библиотека пишет строку только там, где сломалось то, за что никому нельзя
вернуть ошибку: хендлер запаниковал и соединение надо закрывать, ответ не
маршалится, статус не смог унести детали, отказ не удалось записать. Девять мест
на четыре транспорта и общую auth-половину.

Они идут через `Logger`, а не через `log.Printf`, чтобы приложение могло дать им
trace id запроса, направить их в свой хендлер или заглушить — не трогая
общепроцессный логгер, который делят все остальные библиотеки в бинарнике
([[D-062]]). `Logger` никогда не возвращает nil, а `WithLogger(ctx, nil)` ничего
не кладёт.

**Стейтменты — другой вопрос** и другой шов: оборачивайте `crud.Source`. См.
[crud](crud.md#обёртка-над-source--для-трейсинга-таймингов-логов-стейтментов).
