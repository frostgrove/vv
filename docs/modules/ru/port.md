...
объявленные переходы по порядку. `port.WithHops(ctx, hops)` привязывает
неизменяемую копию к одной операции, а `port.HopsFrom(ctx)` читает другую
копию. Сгенерированные биндинги делают это до вызова сервиса, поэтому внешний
или resource renderer получает одну и ту же цепочку перед своим fallback. Не
передавайте эти же сгенерированные hops ещё и через `WithResolvers`: иначе
неидемпотентный пользовательский resolver применится дважды.

### Вывести её вместо того, чтобы печатать

Для ресурса, смонтированного **прямо на модель** — маппера нет, значит модель
*и есть* форма провода — каждая запись выше это переписанный руками тег `json`.
Переписывание руками и есть то место, где для структуры со словом
`sourceFilename` набирают `sourceFileName`. `port.Paths[M]()` читает тег сам
([[D-071]]).

```go
var ContractPaths = port.Paths[Contract]().
    From("json", "form").                                        // предпочтение, не слияние
    Override(port.PathMap{"SourceHash": port.At(FieldFile)}).     // единственный ключ не из тега
    MustBuild()
```

| Метод | Что делает |
|---|---|
| `From(tags...)` | какие теги читать, в порядке предпочтения. По умолчанию `json`; годится любое имя тега, включая собственное |
| `Override(m)` | заменить то, что говорит тег, для названных колонок — и запись всё равно проверяется |
| `Except(names...)` | убрать колонки из карты *и* из проверки полноты |
| `OrFieldName()` | сопоставить колонке, которую **не называет ни один тег**, имя её Go-поля |
| `Build()` / `MustBuild()` | вывести, проверить через `NewPathMap` и ответить или отказать |

**Всё, что не удалось прочитать, — отказ, а не догадка.**

| В модели | Ответ |
|---|---|
| `json:"weight,omitempty"` | `weight` — опции отрезаются |
| `json:",omitempty"` | `Weight` — имя поля, потому что именно так поступает `encoding/json` |
| тега `json` нет вовсе | **отказ**, если не вызван `OrFieldName()` |
| `json:"-"` | **отказ**, и `OrFieldName()` его *не* покрывает — нужен `Except` или `Override` |
| override на колонку, которой нет, или на `generated` | **отказ**, от `NewPathMap` без изменений |

Последние две строки — самое существенное. Молчаливый откат к имени Go-поля
ответил бы `SourceFilename` там, где провод говорит `sourceFilename`: карта
выглядит полной, проходит все проверки и оказывается неверной только в теле
ошибки на проде. А `json:"-"` — это автор, сказавший, что колонки на проводе
нет, и это другое утверждение, нежели молчание о ней.

**Не для сгенерированного ресурса.** `cmd/vv -adapter` даёт своему входу
собственную форму провода (`lowerFirst(FieldName)`, а не ваш тег), поэтому та
карта генерируется вместе с маппером, который она обращает ([[D-050]]).
...
`Violations` сначала применяет `HopsFrom(ctx)`, затем
`ViolationOptions.Resolvers`. Явное поле `Resolvers` предназначено для
написанного вручную транспорта или дополнительного перехода, которым владеет
renderer; сгенерированные биндинги уже передали цепочку service/mapper через
context.
