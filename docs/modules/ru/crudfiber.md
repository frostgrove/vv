...
## Собственные отказы роутера

Путь, который никто не занял, глагол, которого у маршрута нет, и тело сверх
лимита приложения приходят как `*fiber.Error` — до того, как отработает хоть один
хендлер. В таблице сентинелов для них ветки нет, поэтому раньше все они падали в
500, а 404, отрендеренный как 500, читается как авария, и клиент его повторяет.

Теперь их обрабатывают оба шва. `Errors()` и `ErrorHandler()` рендерят отказ
роутера в том же конверте, под статусом, который Fiber уже выбрал, с кодом, по
которому клиент может ветвиться: `not_found`, `method_not_allowed`, `too_large`.
Fault побеждает `*fiber.Error`, который он оборачивает: fault — это голос
приложения, а обёртка — нет.

Собственное сообщение роутера отбрасывается. Это фраза, собранная из пути,
который прислал вызывающий, а действует клиент по коду и статусу ([[D-044]]).
