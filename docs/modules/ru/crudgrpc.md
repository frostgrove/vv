...
поэтому он не может точно нести каждый `int64`. API считает целое значение по модулю 2⁵³ и выше вне безопасного диапазона `Struct`; документ сущности с таким значением объявляет `json:"id,string"`. **Ключи —
...
Query-документ выбирает безопасную сторону: integral number из `Struct` вне
этого точного диапазона отклоняется, а не округляется в другое значение
filter-а. Передайте operand десятичной строкой — обычный query coercion сохранит
его точно. Это относится и к filter-ам внутри `preload`. То же правило у
числового `id` или `ids`: большой ключ должен быть десятичной строкой, а не
`Struct` number.

`page`, `limit`, `offset` и `preload.maxRows` — это управляющие поля, а не
типизированные query-операнды. Они должны прийти точными JSON-целыми; строковая
форма намеренно не принимается. `remote.Resource`, использующий gRPC-транспорт,
при bulk-удалении автоматически отправляет точное десятичное строковое
представление ключей, как и на маршруте с одним ключом. Сырой клиент `Struct`
сам передаёт unsafe `ids` десятичными строками.

`count`, `deleted`, `total` и `totalPages` тоже точны. Значения в безопасном
диапазоне идут JSON числом; `int64` вне его идёт своей десятичной строкой,
потому что соседнее округлённое число хуже явного точного представления.
