...
go run ./cmd/migrate flush                   # все схемы, которыми владеет подключение
go run ./cmd/migrate flush --public          # только схема по умолчанию
...
`flush` — команда восстановления для разработки, когда история Goose ссылается
на уже удалённый файл миграции и `fresh` не может найти его секцию `Down`.
Она удаляет все объекты, включая таблицу истории Goose, и **не** запускает
миграции после этого. Если нужна заново собранная БД, затем явно запусти
`migrate`.

В PostgreSQL команда достаёт **все схемы, которыми владеет подключённый
пользователь**, а не только ту, что в search_path: подсистема, держащая свои
таблицы в отдельной схеме, тоже сбрасывается — именно это делает команду полным
сбросом, а не половиной. Схема по умолчанию пересоздаётся пустой, остальные
остаются удалёнными, потому что тот, кто их создаёт, создаст их снова. Схему,
принадлежащую расширению, команда не удаляет никогда: PostGIS и TimescaleDB
держат в своих схемах каталоги, которые не восстановит ни одна миграция.
Принадлежность читается из `pg_depend`, а не угадывается по имени.

| Флаг | Что сбрасывает |
|---|---|
| *(нет)* | все схемы, которыми владеет подключённый пользователь |
| `--public` | только схему по умолчанию |
| `--schema=a,b` | только перечисленные; отсутствующая в этой БД пропускается, и команда об этом сообщает |
| `--yes` | отвечает на подтверждение, для скриптов и CI |

Перед удалением команда печатает, что именно она выбрала, и спрашивает
подтверждение. Запуск без терминала отказывается работать без `--yes`, поэтому
скрипт никогда не снесёт схему, которую не называл. `--schema` и `--public`
существуют только для PostgreSQL и на других движках отвергаются, а не
игнорируются: в MySQL схема — это база, и их поддержка означала бы снос чужих
баз. В MySQL/MariaDB удаляются все таблицы, представления, процедуры и события
выбранной БД; в SQLite — все несистемные таблицы, представления и триггеры.
Команда разрушительна и должна указывать только на локальную dev-БД.

Генерируемые table-миграции используют `CREATE TABLE IF NOT EXISTS`. Поэтому
init-миграцию можно безопасно повторить, если после неудачного локального
запуска таблицы остались; отличающуюся от модели схему эта конструкция не
сверяет и не исправляет.
...
`rollback`, `fresh` и `flush` открывают только primary, даже если в конфигурации есть
read replica.
