...
go run ./cmd/migrate flush                   # удалить все локальные объекты БД
```
...
Она удаляет все объекты активной схемы/БД, включая таблицу истории Goose, и
**не** запускает миграции после этого. Если нужна заново собранная БД, затем
явно запусти `migrate`. В PostgreSQL пересоздаётся текущая несистемная схема;
в MySQL/MariaDB удаляются все таблицы, представления, процедуры и события
выбранной БД; в SQLite — все несистемные таблицы, представления и триггеры. Команда разрушительна и
должна указывать только на локальную dev-БД.
