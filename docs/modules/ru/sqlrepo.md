...
| `Create(ctx, *M)` | только INSERT — занятый ключ даёт 409, а не перезапись |
| `Replace(ctx, *M)` | upsert по первичному ключу с проверкой `version`; строка, которую кто-то уже сдвинул, даёт `crud.ErrStaleVersion` |
...
Upsert попадает ровно в ту строку, которую называет ключ, и ни в какую другую.
Там, где одним statement это не пообещать — MySQL и MariaDB, у которых
`ON DUPLICATE KEY UPDATE` срабатывает на любом unique-индексе, либо любой
репозиторий с объявленным `Scope`, — `Save` выполняет `UPDATE`, суженный ключом
и скоупом, затем пробу, и вставляет только если строки не было; всё это в одной
транзакции. На PostgreSQL и SQLite без `Scope` это по-прежнему один statement.
...
### Create и Replace договаривают то, о чём Save молчит

```go
stored, err := users.Create(ctx, &u)   // только INSERT; занятый ключ — 409
stored, err := users.Replace(ctx, &u)  // upsert, отказ если u.Version отстал
```

`Save` остаётся значением по умолчанию, и его вызывают все транспорты. Эти два
глагола — для вызывающего, которому нужен явный отказ: `Create` никогда не
поглощает конфликт, а `Replace` привязывает запись к `version` из модели и
увеличивает его. `Patch` среди них нет — им уже является `Update`.

Оба — опциональные capability. Декоратор, который их не пробрасывает, заставляет
вызов вернуть `crud.ErrNoCreateSupport` / `crud.ErrNoReplaceSupport`, а не тихо
дотянуться мимо него: явный глагол не может стать обходом `security.Gate`.
...
| `Scope(pred)` | предикат, добавляемый через AND к каждому чтению, каждому удалению и к строке, до которой вправе дотянуться запись — включая update-половину `Save`/`SaveAll` и чтение строки обратно после неё. Значения новой строки он не сужает |
...
метод. Сгенерированный файл об этом молчит, а `RelPath()` — форма, которую
ничто не затеняет.
...
- **`DeleteAll` перед удалением выбирает свои жертвы с сужением вызывающего**,
  так что сужение декоратора применяется и к удалению, а не только к выборке
  ([[D-026]]).
- **Запись по фильтру отказывает в том, что не может применить.** `Update`,
  `UpdateAll` и `DeleteAll` принимают `Where`, `NarrowRelations`, `ForUpdate` и
  `PrimaryOnly`;
  `Limit`, страница, курсор, сортировка, проекция или preload — это
  `*crud.SchemaError` с именем опции, а не запись во все подошедшие строки
  ([[D-087]]).
