...
## Routing — собственные отказы роутера

| | |
|---|---|
| `Routing(engine, opts…)` | ставит `NoRoute` и `NoMethod` и включает `HandleMethodNotAllowed` |

Путь, который никто не занял, и глагол, которого у маршрута нет, отвечает сам
Gin — до того, как отработает хоть один хендлер или middleware этой библиотеки:
голый 404 без тела, в котором клиенту, разбирающему одну форму на все отказы,
разбирать нечего. Хуже того, приложение, отображающее неизвестные ошибки в 500,
превращает «вы попросили то, чего нет» в «сервис сломан», и клиент это повторяет.

`Routing` рендерит оба в том же конверте, с кодом, по которому клиент может
ветвиться: `not_found` и `method_not_allowed`. `HandleMethodNotAllowed`
включается, потому что без него Gin отвечает на известный путь с неизвестным
глаголом кодом 404 — а для клиента это разные утверждения.

Вызывайте один раз, на движке, после того как маршруты смонтированы.
