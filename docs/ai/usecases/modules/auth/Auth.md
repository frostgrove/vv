...
- **[[D-056]]** — the refusal names no reason to the client. The reporting half
  of this is built and the name is now taken: `auth.Reason` is the *type* an
  `auth.Observer` is handed, and a recovery helper proposed here would have to be
  called something else. `auth.Refuse(cause)` is still unbuilt. Both are
  server-side and must never be reachable from a renderer; the control case at `auth/errors_test.go:82` is what would catch the
...
| 9 | **half closed** — nothing reported the reason; now `auth.Observe(observer)` hands every refusal to an `auth.Observer` as an `auth.Reason` (kind, detail, error — never a credential), so all four transports report identically and `auth.Sampled` bounds a stuffing run. Still open: `auth.Refuse(cause error)` for a consumer's own refusal, and the recovery helper, which now needs a name other than `auth.Reason` | sharp edge | [[UC-019]] guarantee 6 promised recoverable and delivered it; the reported half was the authhttp sweep's blocker 4 and is closed with it — one incident, two halves, counted once. The remaining half is a consumer wrapping their own cause into the flat 401, which no observer does for them. |
