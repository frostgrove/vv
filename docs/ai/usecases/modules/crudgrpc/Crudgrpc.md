...
- CLAUDE.md's triplet rule does **not** propagate a change out of this module:
  ...
  and `scripts/checks.sh:TRIPLETS` lists only the two HTTP triples, so `make
...
| 7 | Eleven test names the three HTTP bindings each carry are missing here — five for `WithScope` including its asymmetry control, four for `BeforeSave`/`BeforeUpdate`, `TestWithTransformAppliesToWritesToo`, and `TestAllowClientIDLetsTheClientChooseTheKey` | serious | `crud/rpc/crudgrpc` | `make check-triplets` cannot see this module (`scripts/checks.sh:TRIPLETS`), so nothing mechanical holds the parity. If the presenter stopped running on writes, a `Create` response would echo the columns it exists to hide and nothing here would fail. |
