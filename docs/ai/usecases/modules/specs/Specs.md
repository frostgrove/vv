...
Guarantee 4 now holds directly. `WithNullsLast` and `WithNullsFirst` use native PostgreSQL grammar and a leading `column IS NULL` key on MySQL/MariaDB/SQLite. Unit coverage pins the rendered forms, while `TestNullOrderingUsesEngineDefaultsUntilTheCallerChoosesAPortableHint` first proves the engines' opposite no-hint defaults and then proves both explicit choices agree live. The emulation can affect index use, which [[D-019]] documents, but the requested result is no longer discarded. `Asc`/`Desc` remain declared on `Attr` and promoted rather than on `Str`/`Ord`.
