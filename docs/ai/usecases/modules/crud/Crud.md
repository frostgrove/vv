...
**Today:** ✅ ready
**Evidence:** `BindExecutor`, checked `Session`, strict legacy resolution and the
explicit `WithUnsafeExecutor` escape hatch are [[D-082]] and [[FL-009]]. Unit
tests pin nested/multi-database matching, wrapper identity, strict mismatch and
transaction-source refusal. Live database/sql and pgx tests prove rollback and
show that neither natural wrong spelling falls through to the pool. [[UC-012]]
and [[UC-005]] cover the consumer contract end to end.
**If not ready:** —
