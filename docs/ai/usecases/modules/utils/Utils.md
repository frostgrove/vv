...
**Today:** ❌ missing
**Evidence:** cleanenv decodes YAML with `yaml.NewDecoder(r).Decode(str)` and never calls `KnownFields(true)` (`cleanenv.go:158-160`); JSON goes through `json.NewDecoder` with no `DisallowUnknownFields` (`:164-165`); `ReadConfig` is the only door `vvcfg` uses (`utils/vvcfg/vvcfg.go:61`). Unknown keys are dropped in silence. [[D-013]] records the same shape one subsystem over — `{"filtr": {…}}` decoded into a request with no filter and answered 200 with the whole table — and closed it there. It does not reach a configuration file on its own; see **What it must not break**.
**If not ready:** The consumer catches it in `Validate()` if the field happened to be required, and does not catch it at all otherwise. Closing it means `vvcfg` decoding the file itself for the three formats that have a strictness door, and saying plainly what happens in `.env` and `.edn`, which do not — and the module doc does not currently admit `.edn` exists at all (H-UTILS-20).
...
**Today:** ❌ missing
**Evidence:** `Auto` calls `Find` and returns its error (`utils/vvcfg/vvcfg.go:74-80`), and `Find` returns `ErrNoPath` when neither source names a file (`:36-46`); `Load` refuses an empty path outright (`:50-52`), pinned deliberately at `vvcfg_test.go:112-114`. Nothing exported reaches cleanenv's `ReadEnv` (`cleanenv.go:107-109`). Must-holds 1 and 2 are the same code path and different promises, which is exactly what an `Optional()` option's doc comment has to choose between; see **Why this shape**. The sibling sweep agrees rather than conflicting: its H-VVDB-02 must-hold 4 is graded "another module's and is broken there" and defers here (`docs/ai/usecases/modules/vvdb/Vvdb.md:91-92,103-109`). Its pointer back is stale — it says "H-UTILS-02, blocker 4" where this file has H-UTILS-05, blocker 5 — and that is the one real cross-file defect in the pair; fixing it belongs to whoever closes this row.
**If not ready:** The consumer imports `cleanenv` directly — `cleanenv.ReadEnv(&cfg)` — and re-implements the `Validator` hop themselves, three or four lines, taking a direct dependency on the library this module exists to wrap. Closing it is an option that makes "no source named a file" a legal outcome rather than an error, on `Find` and `Load` both, and one sentence saying which of the two promises it makes.
...
**Today:** ❌ missing
**Evidence:** `utils/vvcfg/vvcfg.go:64-68` asserts `Validator` on `&cfg` and stops there; nothing walks the fields, so (1), (2) and (3) all fall at the same line. `utils/vvdb/config.go:138-141` is the workaround written down — "an application whose config embeds this one **forwards** to it" — but only half of that comment is true: its first sentence claims `Validate` "is called by [DSN] and by [Open]", and it is called by nothing in the package, which is the sibling sweep's blocker 2 (`docs/ai/usecases/modules/vvdb/Vvdb.md:877`). What is lost here is narrower than "the DSN rule stops running": `prepare` re-checks `fieldsBesideDSN` and `validateFields` on its own (`utils/vvdb/dsn.go:179-190`) and `DSN` refuses an unknown engine (`:28`), so a contradictory `db:` block still cannot reach a connection — it fails later, at `Open`, in a message that does not say which block was wrong. What lives *only* in `Validate` is the replica pair (`utils/vvdb/config.go:153-165`): the engine-mismatch check, and validating the merged replica rather than the fragment. Since `ReadReplica` lets a replica override the engine (`:246-248`), a replica declaring `engine: mysql` under a Postgres primary opens a MySQL handle that then gets addressed in Postgres's dialect. The embedding case is the sharp one: promotion makes `any(&cfg).(Validator)` succeed — `Validate` has a value receiver (`config.go:142`) — so the loader validates the database block until the day the author writes their own `Validate`, and then silently stops.
**If not ready:** The consumer writes `func (c *Config) Validate() error { return c.DB.Validate() }` and remembers to extend it for every block they add, and still gets one failure per restart. Closing it is a reflective walk of the exported fields joining every failure, rules stated in **Why this shape**.
...
**Today:** ❌ missing
**Evidence:** `Auto` discards the path (`utils/vvcfg/vvcfg.go:74-80`), and `Find` returns the path but not which of the two sources supplied it (`:36-46`) — the two branches return the same shape. So the ordinary start-up line cannot say "from `CONFIG_PATH`", which is the half that matters: a flag is visible in the pod spec and an environment variable usually is not. This is the survivable half of H-UTILS-18. The current answer is to drop a level and call `Find` yourself, which the module doc does not suggest and which loses the source anyway. It is also not sufficient on its own: H-UTILS-22 is why.
**If not ready:** The consumer calls `Find` and `Load` separately and logs the path, and still cannot say where it came from. Closing it is a second result on `Find` — a signature change, so now or never.
...
**Today:** ❌ missing
**Evidence:** nothing found for this. `Load` returns the struct and nothing else (`utils/vvcfg/vvcfg.go:69`), and cleanenv's `GetDescription` (`:569`) describes the *declaration* — variable names, kinds, defaults — not the resolved values, and skips every field with no `env` tag (`:586-588`). This case exists because blockers 7, 8 and 11 together mean the filename does not determine the configuration: a correct `/etc/app.yaml` plus one stray `DB_HOST=` in the pod spec produces the wrong database with the right filename in the log, and on-call reads the log line, believes the file, and goes looking in the wrong place. (2) is not this module's to hold alone — a renderer is only as safe as the field types it walks, and `vvdb.Config` prints its password today, which is H-VVDB-10 in the sibling sweep.
**If not ready:** The consumer writes `log.Printf("%+v", cfg)` and ships the password, which is the most ordinary line in Go. Closing it is a renderer over the loaded struct plus a redaction contract the field types opt into — and the redaction half has to land in `vvdb` first, or the feature is a leak with a nice API.
...
| Exported surface | 6 names in `vvcfg` (`docs/api/surface.md:872-877`), 4 in `vvflag` | ~16 in `vvcfg`, and one signature (`Find`) changed — the only part that is now-or-never |
...
| No file is a legal deployment | not reachable; the consumer imports cleanenv directly and re-writes the `Validator` hop, 3–4 lines plus a direct dependency (#5) | small |
...
| Nested blocks validate themselves | one forwarding method per nested block, extended by hand forever; the embedded case stops the day the author writes their own `Validate` (#6) | large |
| Every *validation* failure at once | first failure only (#6, same fix) | small; the decode and environment passes stay first-failure unless `vvcfg` owns them |
| Unknown key in the file is refused | nothing; it is dropped in silence (#3) | large — and a decision first |
...
| The start-up log says what the process is running with | `Find` returns a path with no origin, `Auto` returns neither (#17), and nothing renders the resolved values (#13) | small for the origin, large for the renderer |
...
| 3 | An unknown key in the YAML is silently ignored | serious | A renamed key boots on defaults with no way to see it; [[D-013]] records the identical shape one subsystem over, though extending it here is a decision the owner has to make |
...
| 13 | Nothing renders the effective configuration, so the start-up log can name the right file and still describe the wrong configuration | serious | With #7, #8 and #11 live, the filename does not determine what the process is running with; on-call reads the file name, believes it, and looks in the wrong place |
...
| 17 | `Find` discards which source named the path and `Auto` discards the path | sharp edge | The start-up line that would make #1 survivable in production cannot be written — and `Find`'s signature is the one thing here that cannot change after the tag |
...
| 26 | Neither package owns a flow doc, a use case, a running example or any integration coverage | sharp edge | The only stated guarantee about this loader lives in [[UC-021]], another module's use case, where nobody changing `vvcfg` will look; `grep -rn vvcfg docs/ai/flows/` is empty and `_examples` names it in one comment |

**Housekeeping found on the way, not a blocker:** `gofmt -l` is not silent, and
it is five files, not three: `utils/vvcfg/vvcfg.go`, `utils/vvdb/dbpgx/dbpgx.go`,
