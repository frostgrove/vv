...
| 16 | **Closed:** `nulls: "last"` / `"first"` uses native PostgreSQL grammar and a boolean null-key on MySQL/MariaDB/SQLite | resolved | Unit SQL assertions and a live cross-engine control prove defaults may differ but every explicit placement agrees; [[D-019]] records the possible index cost |
