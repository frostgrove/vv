...
**Today:** 🟡 partial — (1), (3), (4), (5) and (6) hold; (2) has one exception left.
**Evidence:** (1) is `TestScopeIsAppendedToEveryRead` (`security_test.go:63`) for the row reads and `TestCountAndExistsNarrowTheirSubqueries` (`relscope_test.go:198`) for the two scalar ones. The scope is *prepended*, so `Where` ANDs and cannot be subtracted (`crud/decorators/security/security.go:scoped`), which is (6). (5) is `TestTheGateScopeIsInTheUpdatesOwnWhereClause` and `TestAnUpdateOfARowThatLeftTheScopeIsNotFound` (`gate_edge_test.go`) over `crud/decorators/security/security.go:Update`, which passes the scope as an option rather than only checking it — the check-then-act failure that comes from is written down in [[D-008]] and [[FL-008]], and nowhere in the code, because this repository does not comment code that short. `Aggregate` is overridden rather than inherited and the reason is in the refusal it returns (`crud/decorators/security/security.go:Aggregate`); `TestEveryVerbOnTheSeamIsGatedOrHasAWrittenReason` (`obligation_test.go:104`) fails the build if a thirteenth verb reaches the seam and nobody decides what the gate does with it, with `TestTheInheritedVerbsAreNotGated` (`obligation_test.go:156`) as its control so "inherited" means something. 404-not-403 on `GET`, `PATCH` and `DELETE`: `crud/decorators/security/security.go:loadScopedWith`, `TestAnIDInAnotherTenantIsInvisibleRatherThanForbidden` (`edge_test.go`).
`PUT` used to be the first exception and is not one any more. `saveTarget` still probes for the row *without* the narrowing, because a client-owned key that already belongs to somebody else must not be written; but when a scope is declared the answer it gives back is `crud.ErrNotFound`, so the transport renders 404 and the probe reveals nothing (`security.go:639-666`, and the same for the two scoped-upsert paths at `:597-637`). `TestSaveOfAnotherTenantsAssignedKeyLooksMissing` (`gate_edge_test.go:118`) asserts both halves — not-found comes back, forbidden does not — and `TestSaveRefusesToOverwriteATombstoneHiddenByRepositoryScope` (`:97`) says the same for a row hidden by soft delete instead of by tenant. `PUT /articles/{another-tenant-id}` now answers what `GET` on the same id answers. The exception that is left is bulk delete, which is not deliberate and is H-SECURITY-20.
**Cost, since the module reference does not state it:** `ScopeAttr` installs an `Inspect` (`policies.go:34-51`), so every `PATCH` pays a `SELECT` before the load-diff-write and every `Save` carrying an id pays a scoped lookup plus, on a miss, an unscoped presence probe (`security.go:639-666`). Dropping `Inspect` does not buy that back: a scope-only policy is refused at `Save` outright rather than run cheaply (`security.go:524-526`, `TestAScopeWithoutInspectRefusesSaveBeforeItCanWrite` at `gate_edge_test.go:26`, with `TestAScopeWithoutInspectRefusesEveryWriteWithABody` at `:55` for the other verbs). The per-row cost is the price of writing through this gate at all, not a shape the consumer can decline.
**If not ready:** nothing left to write by hand for the `PUT` asymmetry. What the module reference still owes a consumer is the cost sentence above — that a scoped write is two statements, not one — so it is found in a load test rather than in production.
...
5. The permission my transport publishes for a route is the permission this map enforces for that route's verb — not a second list I keep in step by hand.
**Today:** 🟡 partial — (1), (2), (3) and (5) hold; (4) is deliberate and goes the other way for one of the two quantifiers.
**Evidence:** `PerAction` fills `Policy.Requires`, which `requireDeclared` enforces before any other rule and which refuses an unnamed action outright; the map is copied at declaration so a later write cannot change what is enforced (`principal.go:53-59`, `:61-74`, `:76-83`), pinned by `TestPerActionRefusesAVerbNobodyDeclared` (`principal_test.go:107`); that default-deny plus `TestEveryVerbOnTheSeamIsGatedOrHasAWrittenReason`, which asserts `len(rec.Statements()) == 0` per verb, is (2). Every rule in `principal.go` calls `auth.Require` first — `requireDeclared` at `:62`, the two quantifiers at `:26,41`, `attrOf` at `:112`, and `InspectOwner` at `:138` — which errors on a context with no principal (`auth/context.go:45-51`); `TestEveryPrincipalPolicyFailsClosedWithoutOne` (`principal_test.go:227`) walks them.
(5): the same map is readable as `Policy.RequiredFor(action)`, and `crudhttp.Table.GuardedBy` builds the route declaration from it rather than from a second list; a mounted route whose verb the map does not name is refused where the declaration is assembled ([[D-107]]). Pinned by `TestTheDeclaredRequirementIsTheOneTheGateEnforces` (`principal_test.go`) and, per binding, `TestTheAccessDeclarationIsDerivedFromTheGatesOwnPermissions` and `TestAMountedRouteThePolicyLeavesUndeclaredIsRefusedAtAssembly` (`crud/http/crud{net,fiber,gin}/table_test.go`).
(4): `RequirePermission()` with no permissions refuses nothing and `RequireAnyPermission()` refuses everything, both on purpose and both written down (`principal.go:15-21`, `:23-36`, `docs/modules/en/security.md`, the two-quantifier bullet). A permission list built from configuration that comes back empty therefore produces, in the first spelling, a policy that authorises every verb — the fail-open direction, in the one layer whose job is to fail closed before any SQL. It does not reach the blocker table because it is documented twice and the composed policy's scope still narrows; it is here because the doc that explains it is not the doc a consumer reads while wiring config.
...
**Today:** 🟡 partial — (1), (2), (3) and (4) hold; (5) is false.
...
(2) held only for the unpaged call until [[D-026]] was settled; both halves are now closed. The victim reads append `inspectionRead()`, which zeroes `Limit`, `Page`, `Offset` and both cursors, so the scan sees every matching row rather than the first page of them; and the repository refuses a paging, sorting, projection or preload option on `UpdateAll`, `DeleteAll` and `Update` outright, so a caller who passes `crud.Limit(1)` gets a `*crud.SchemaError` and no write at all ([[D-087]]). `TestAGatedFilteredWriteRefusesPagingRatherThanWritingEveryRowItShowedTheRule` (`updateall_test.go`) is the test D-026 asked for, with the unpaged control beside it. Not reachable from HTTP either way — no binding exposes either verb.
...
(5): because `ScopeAttr` sets `Inspect`, `DeleteAll` reads every matching row — whole rows, projection cancelled by `whole` (`security.go:247-252`, called at `:728`), on the primary — into a slice before the `DELETE` runs. A cleanup job over a large archive is an out-of-memory, not a slow query, and a `Limit` cannot bound it: the statement never carried one and the repository now refuses the option rather than accepting it ([[D-087]]). [[D-026]] names the unbounded scan as the cost of its own option 1, and that cost is the part still outstanding.
...
**Today:** 🟡 partial — a claim the column cannot take is refused instead of served, in both directions; what is still missing is that the refusal arrives at the first request rather than at declaration, and (2) is still out of reach.
**Evidence:** names are resolved at declaration; value types are not. `reconcile` runs inside the `Scope` and `Inspect` closures (`policies.go:74-89`, called from `:23-33` and `:34-51`), so the verdict on a type arrives with the first request that carries one. It is a verdict rather than a crash: `reconcileFieldValue` answers `Denied` both for a nil value and for one it cannot safely convert, and `safelyConvert` (`policies.go:91-140`) takes only a same-kind conversion or an integer that survives the round trip. `TestAnUncomparableClaimTypeFailsClosed` (`policies_test.go:51`) proves the refusal and that no statement went out behind it.
**The two silent conversions are closed.** An integer **is** convertible to a string in Go — as the rune conversion — so a `"tenant": 42` claim against a `tenant_id varchar` column used to compile to `WHERE tenant_id = '*'` and stay there. `safelyConvert` refuses a cross-kind conversion outright and a float that would round, pinned by `TestRelationScopeRejectsUnsafeNumericToStringConversion` (`policies_test.go:100`) and `TestScopeFieldRefusesALossyFloatTenantConversion` (`:119`), with `TestRelationScopeRejectsANilValueBeforeItReachesTheDriver` (`:90`) for the claim that is absent altogether. A JWT claim decodes to `int64` or `string` (`auth/authjwt/claims.go:64-95`), and neither reaches a `uuid.UUID` column by conversion any more; the caller is told.
(2) still fails, and now fails as a refusal: `attrOf` reads one flat claim name (`principal.go:105-117`), so a nested claim comes back as a `map[string]any`, which converts to nothing and is denied. Nobody is served the wrong rows, and nobody can use a nested claim either.
The near-miss is on record in [[D-021]]: the shipped gorm guide's own `ScopeAttr[Member, uint]("TenantID", "tenant")` against an `int64` claim was a working reproduction of a third variant — reads fine, every create denied — and `TestAClaimOfADifferentWidthThanTheColumnStillWorks` (`policies_test.go:25`) closed it.
**If not ready:** nothing to write by hand — a wrong claim type is refused. What is left is [[D-021]]'s standard: the magic must fail at build or start-up, and this is the one place in the package that decides at request time. Closing that means resolving the value type at declaration, which the extractor's `any` signature does not allow — so a typed constructor (`ScopeAttrAs[M, ID, V]`), or reaching the claim shape from the token schema. For (2), `ScopeField` with an extractor that walks the nested claim is five lines and correct.
...
**Evidence:** `subject` always answers a `string`, because `Principal.Subject()` is a `string` (`principal.go:191-200`, `auth/principal.go:26`). `reconcile` then requires that string to convert safely to the column's type (`policies.go:74-140`), and a string reaches neither an integer nor a `[16]byte` that way. So `ScopeSubject[Order, int64]("UserID")` compiles, starts and denies every request, exactly as H-SECURITY-15 describes — with a much larger blast radius, because an integer user id is the more common schema, not the rarer one. The only test uses a string column: `principal_test.go:215` declares `Owner string` and `TestScopeSubjectNarrowsToTheCallersOwnRows` (`:223`) asserts the argument is `"u-1"`. Nothing catches the ordinary case.
...
- **[[D-026]] — the victim fetch carries the caller's options, and it is
  settled.** H-SECURITY-07's `Limit` edge was this decision. It took two of its
  three options — the fetch strips paging, the repository refuses it
  ([[D-087]]) — and it still forbids making `DeleteAll` honour a `Limit` to make
  the two agree. What is left of it here is the unbounded victim scan.
...
| 8 | The whole victim set of a filtered write is materialised in Go, and no option can bound it | sharp edge | [[D-026]] settled the truncation half — the scan strips paging and the repository refuses the option ([[D-087]]) — and the unbounded read that costs is an OOM on a cleanup job. Only reachable from Go |
...
| 14 | `[[FL-007]]` and `[[FL-008]]` cited 40 line offsets into `security.go` and `policies.go`, essentially all stale by 40 to 180 lines, and `docs/ai/usecases/Index.md` still listed as open four things the code had closed | fixed | Both flows now anchor on `file.go:Symbol`, which does not rot, and `TestEverySymbolTheDocsCiteIsDeclaredWhereTheDocSaysItIs` (`scripts/docs_test.go`) fails the build when one of those symbols moves out of the file the doc names. The index entries are marked closed with what settled them. What is left is the rest of `docs/`, which still cites line offsets nothing checks — including this file |
