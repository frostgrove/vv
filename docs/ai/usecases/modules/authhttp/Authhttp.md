...
**Today:** 🟡 partial for the cookie · ❌ missing for the query string
...
| A credential in a cookie, a query string or a WebSocket upgrade | Cookie: a 6-line `Lookup` a consumer invents, with replace-semantics nobody warned them about. Query string: unreachable from every getter; the shape is a pre-tag decision | medium |
