...
4. What a preflight buys is an answer, never a route: no request that skipped the guard reaches a handler.
**Today:** ✅ closed — `authhttp.Preflight` knows exactly what a preflight is, and each binding carries `AnswerPreflight` and its argument-free `SkipPreflight`
**Evidence (when this was open):** `grep -rni "cors\|preflight"` and `grep -rn "MethodOptions"` over the whole repository both came back empty — no code, no test, no module page, no flow. A required guard refuses a request that presents no credential at `auth/guard.go:100`; the `if g.optional` branch two lines above (`:97-99`) is the only exception, and an SPA does not mount an optional door on `/orders`. Every binding mounts in front of routing, so `OPTIONS` reaches it like anything else. The one auth example mounts globally (`_examples/auth-jwt-gin/main.go:152`) and has no browser client, so it never meets this.
**How it was closed:** ordering is still the recommended fix and is now written down on each binding's page. Beside it, `authnet.AnswerPreflight`, `authgin.AnswerPreflight` and `authfiber.AnswerPreflight` decorate the middleware for the chain a consumer does not order — the composition root of a framework that mounts auth for them. Must-hold 3 is intact: this is not an exemption list. The one argument it takes is *who answers a preflight*, not *which paths are exempt* — there is still no path it can be pointed at, and a reviewer audits one predicate and one handler. Must-hold 4 is what the argument buys: the wrapper answers with that handler and ends the request, so the `OPTIONS` route somebody wrote by hand is never reached by a request that presented nothing ([[D-103]]). `SkipPreflight` keeps the argument-free mount and answers `204` itself. `authhttp.Preflight` matches one shape — `OPTIONS`, an `Origin`, an `Access-Control-Request-Method`, and no `Authorization`. An `OPTIONS` carrying a credential is authenticated like any other request. Pinned by `TestACorsPreflightIsAnsweredByTheHandlerNamedForItAndABareOptionsIsNot`, `TestAPreflightNobodyAnsweredStopsAtTheDoorInsteadOfAtTheRoute` and `TestAPreflightAnswerThatIsNotThereRefusesToStart` on all three bindings, each with its controls.
...
| 3 | ~~A CORS preflight is refused by a globally mounted door~~ **closed** — `authhttp.Preflight` and the three `AnswerPreflight`/`SkipPreflight` decorators, with the ordering answer now written on each binding's page | closed | It was not fixable by an exemption list and still is not; what closed it is a predicate over one shape rather than a path list, so blocker 5 is unaffected. The decorator answers a preflight instead of routing it ([[D-103]]), so closing this one did not open a door of its own. |
| 4 | ~~The refusal reason reaches no log on any transport~~ **closed** — `auth.Observe` puts an `auth.Observer` on the guard itself, so every transport reports the same `auth.Reason`, and no binding decides for itself | closed | The half `Auth.md` blocker 9 owns is the same seam: `Reason` carries a kind, a detail written in `auth` and the error, never a credential. `auth.Sampled` is the decorator for a 401 storm. Gin's `c.Error` stays where it is — it is Gin's own bag, not this library's answer. |
