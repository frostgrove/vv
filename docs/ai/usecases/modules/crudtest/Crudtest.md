...
**Must hold:**
1. A queued row is assigned to the model the way `database/sql.Rows.Scan` would assign it, with checked driver conversions rather than arbitrary Go conversions.
...
**Today:** 🟡 partial — (1), (2) and (3) hold and are pinned against `database/sql`; (4) holds for arity and **fails for same-typed order**.
**Evidence:** `TestScanFillsAModelThroughTheSchema`, `TestScanUsesDatabaseSQLConversions`, `TestRecorderScanMatchesDatabaseSQL` and `TestScanRefusesRowsItCannotFill` prove (1) and (2); `TestNullableColumnScansToNull` is (3). The parity matrix catches the former arbitrary `reflect.ConvertibleTo` behaviour: overflow, signed-to-unsigned conversion, and int-to-string rune conversion can no longer make a fake pass when `database/sql` would fail or produce another value. Arity is refused loudly. Same-typed column order remains inherently positional and cannot be inferred from `Rows.Scan`; that is the remaining argument for a named row builder.
