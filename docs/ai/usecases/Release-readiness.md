...
| Authhttp | not ready | A stricter mounted guard can silently never run. | [Authhttp](modules/authhttp/Authhttp.md) |
...
| 18 | Authhttp + Auth (merged) | A nested stricter guard is skipped because idempotence records only that some principal exists. | blocker | yes | An `/admin` step-up route looks protected while accepting the outer identity. |
...
| 22 | Adapters | Mutable `*sql.TxOptions` changes transaction isolation/read-only policy after configuration; cursor-close errors are discarded. | serious | yes | A reviewed reservation policy can execute weaker, or a database read can be reported as success. |
