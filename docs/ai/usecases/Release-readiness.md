...
| 7 | Sqlrepo | Versioned full-model `Save` still does not refuse a stale writer, and `DefaultService.Replace` (`PUT`) routes through it. The MySQL half is closed: `Save` no longer emits a targetless upsert, so a row chosen by another unique key is a 409 rather than an overwrite, and `crud.Repo.Replace` is the version-aware verb. | blocker, narrowed | yes | The remaining half is the HTTP replacement route, which still takes the version-agnostic verb. |
