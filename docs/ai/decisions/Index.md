...
deliberate rather than rot. **The set currently contains D-134 and D-136**:
D-134 lists its OTel O1–O3 proof in `Proven by (owed)`, while D-136 has a
...
| [D-048](D-048-the-contract-manifest-is-closed.md) | A package joins the contract manifest only when a second implementation asks, and never when the standard library already contracts it | accepted — i18n example superseded by D-135 | process & tooling |
...
| [D-142](D-142-an-operation-receipt-is-the-applications-table-beside-the-append.md) | A receipt is a durable row in the application's own table, in the event store's database, written through the caller's own transaction behind an interface this framework declares and does not implement; the order is claim, decide, append, complete and the claim is first, because two retries at two expected versions both append and collide with both sets of events already in the log; the claim is `INSERT … ON CONFLICT DO NOTHING` **then** a `SELECT`, in that order, whose serialisation point is the primary-key index plus the level's conflict behaviour; the fingerprint covers the composed stream and the encoded records and deliberately not the version, over a frozen preimage; one key covers one append; and `Store.Append` stays non-deduplicating | accepted | event sourcing, transactions & datasources |
| [D-143](D-143-an-absent-receipt-is-not-a-rollback.md) | An absent receipt is never a rollback: a resolver on a second connection cannot tell a transaction that rolled back from one still open, so that state is `Unresolved` and no lock closes it; a resolve has four standings and a claim three, and they are two enums so neither door can return the other's; a present row with no range is `ErrIncomplete` rather than a fourth verdict; retention bounds the window through a published horizon on the ledger's own database clock, with `Issued` the one cross-clock comparison and optional; and the `pg_current_xact_id`/`pg_snapshot_xmin` fourth standing was designed and refused with its cost | accepted | event sourcing, errors |
| [D-144](D-144-a-bounded-read-is-a-read-and-never-a-load.md) | `Repo.StateAt` answers a state and an error and no second value, so a historical read cannot be the first half of a `Load → Decide → Append`; version zero, a version past the end and an empty stream are one `ErrVersion` and no refusal names a number; there is no timestamp boundary and one could only ever be a lookup resolving to a version; `Store.ReadStream` gains no ceiling because the over-read is one page whatever the stream's length; and none of it widens [[UC-032]], whose clauses are unchanged | accepted | event sourcing, querying |
| [D-145](D-145-a-snapshots-contract-accepted-without-code.md) | A snapshot is bound to backing, stream, version, payload revision and a state-computation version, all five compared **before** its payload is deserialised and never stored inside it; a snapshot is never upcast; the fallback to full replay is observable in the return value and is entered on the snapshot's own failure and no other, so an unreadable event is a refusal; history is never deleted and a snapshot is not history; nothing detects a forgotten computation-version bump and the suite pins that hole with an inverted control; and a snapshot ships only with both gates — a named deployment's own p99, and snapshot+tail ≡ full replay | **accepted — in force from the phase that meets [[D-132]]'s gate; amends D-132, which is not superseded** | event sourcing |
...
**Process & tooling** — D-135 (why full i18n is an optional nested module while
`errs.MessageSource` and the contract manifest stay in the root), D-134 (the only allowed OTel production imports and the
...
**Operations** — D-134 (the admitted semantic telemetry seams, fail-safe signal
...
**Audit** — D-136 (the four distinct evidence products, explicit allowlists and
...
`cache`'s length prefix is the wrong precedent for it), D-128 (why delivery order
is a law rather than a capability, what `Progress.Highest` therefore means, and
why the PostgreSQL reference's tuple read does not transfer to a store that joins
the caller's transaction), D-133 (why a projection that loses the fence takes its
turn rather than halting, why the advance is claimed before the handler inside a
unit, and which lost fences still halt), D-129 (why a checkpoint is a cursor and
never a position, and why a cursor is never ordered), D-130 (why a projection is
a supervised runner, why the framework opens no transaction, why the read is
...
**Event sourcing, phase 5 — the caller's guarantees** — D-142 (why a receipt is
the application's table beside the append, why the claim comes first, why the
claim is two statements in that order, what the fingerprint covers and what it
deliberately does not, and why `jobs.EnqueueOnce` is the convergent mechanism
rather than the same one), D-143 (why an absent receipt is not a rollback, why the
third answer is the point, which clock each instant comes from, and the fourth
standing that was designed and refused), D-144 (why a bounded read yields no
append token, why three refusals are one, why there is no timestamp boundary, and
why none of it widens [[UC-032]]), D-145 (the snapshot contract, accepted without
code: the five bindings compared before deserialisation, the two components of the
state-computation version, the observable fallback, the unreadable event that is
never hidden by it, and the two gates neither of which is met). **D-145 amends
[[D-132]] and does not supersede it** — D-132's invariant and its enforcement test
stand, and D-145 is the contract its re-entry trigger opens into.
