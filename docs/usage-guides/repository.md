...
же write-only поведением. Если пачка не помещается в bind-бюджет диалекта,
репозиторий заранее строит все чанки и выполняет их в одной транзакции. Порядок
сохраняется, а ошибка позднего чанка откатывает ранние ([[D-079]]).
...
Запись по фильтру принимает только сужающие опции — `Where`,
`NarrowRelations`, `ForUpdate` и `PrimaryOnly`. Скомпилированный список из DSL несёт ещё и
пагинацию с сортировкой, а портируемого `UPDATE … LIMIT` не существует, поэтому
такой список целиком даст `*crud.SchemaError` с именем опции, а не запись во все
подошедшие строки. Передавайте предикат, а не запрос.
...
`Delete(ctx, ids...)` — другая массовая форма: набор ключей уже является
сужением. Большой набор автоматически делится по bind-бюджету, причём постоянный
scope репозитория и soft-delete timestamp учитываются в каждом чанке. А вот
произвольный `In` внутри `Where` не чанкуется: несколько запросов не равны одному
предикату. Слишком большой statement получает `*crud.SchemaError` до обращения
к datasource.
...
`Delete(ids...)`: ambient executor присоединяется, новый открывается только при
необходимости нескольких statements. Если source не умеет начать транзакцию,
чанкованная запись возвращает `crud.ErrNoTxSupport`, не выполнив первый чанк.
