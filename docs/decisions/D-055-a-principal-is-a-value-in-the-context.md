...
http/authhttp/        the HTTP-shaped shared half of the middleware  (no dependency)
http/authnet/         the net/http middleware                        (no dependency)

auth/authjwt/         MODULE — golang-jwt/v5
http/authgin/         MODULE — gin
http/authfiber/       MODULE — fiber v3
rpc/authgrpc/         MODULE — grpc
```

`repo/decorators/security` gains `principal.go` and imports `auth`. The import
...
may exist without being on the manifest. `vvflag`, `tools/vvcfg`,
...
reason to compile a repository in. Inverted, `http/authgin` would drag `crud`,
`repo/basic` and the whole predicate AST into a process that only wanted to read
...
`http/authgin` may not be a file in `http/crudgin`: D-035 forbids a subsystem ×
...
- Do not import `repo/decorators/security`, `crud` or `port` from `auth` or from
...
- Do not let a policy read an absent principal as "no narrowing". Every helper
  in `security/principal.go` refuses, which is [[UC-004]]'s guarantee 16.
...
- `http/authhttp/`, `http/authnet/`, `http/authgin/`, `http/authfiber/`,
  `rpc/authgrpc/` — the four transports and their shared HTTP half.
- `repo/decorators/security/principal.go` — the bridge, and the only file in
...
packages with `auth`, `auth/apikey`, `http/authhttp` and `http/authnet` in it,
and `http/authgin` resolves gin without fiber while `http/authfiber` resolves
fiber without gin.

Tests:

- `TestEveryPrincipalPolicyFailsClosedWithoutOne` —
  `repo/decorators/security/principal_test.go`. Every helper, one subtest each,
