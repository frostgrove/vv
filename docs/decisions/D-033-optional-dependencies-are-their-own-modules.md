...
Thirteen modules in one repository, eleven of them published. The list is
...
- `test/go.mod`, `_examples/go.mod` — the unpublished modules, each with a
  `replace` per published module it imports. `utils/vvcfg` is in neither,
  because neither imports it.
- `go.work` — joins the published modules and `test`, regenerated by
  `make work` from what is on disk.
- `Makefile:MODULES` — discovered by `find`, not listed. A hand-written list is
  how a new module escapes unit, vet, tidy and release at once, and this
