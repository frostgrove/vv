...
```
github.com/shardit-io/qq                    crud, query, repo, cmd,
                                                    adapter/crudsql, http/crudhttp
github.com/shardit-io/qq/http/crudfiber     + fiber/v3
github.com/shardit-io/qq/http/crudgin       + gin
github.com/shardit-io/qq/adapter/crudpgx    + pgx/v5
github.com/shardit-io/qq/test               unpublished, replace ../
github.com/shardit-io/qq/_examples          unpublished, replace ../
```
...
- *No `replace` in anything published.* Each submodule's `go.mod` carries a
  plain `require github.com/shardit-io/qq vX.Y.Z`. The only `replace` in
...
- `go.mod` — module `github.com/shardit-io/qq`, `go 1.26`, no `require`
  block at all.
...
```
go list -deps ./... | grep -v '^github.com/shardit-io/qq' | grep '\.'
```
