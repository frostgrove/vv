...
**Invariant:** `go get github.com/shardit-io/ordo` must be the whole installation — no `replace` directive — and no file in `crud/` outside `_test.go` may import anything but the standard library.
...
- `go.mod` — module `github.com/shardit-io/ordo`, `go 1.26`, two direct
...
should print nothing but `github.com/shardit-io/ordo/crud` itself. Adding
