...
PostgreSQL append/deployment/history path form the current alpha base. Public
exact revision/item inspection now has memory and PostgreSQL parity. Protected
...
mutations with evidence. Public one-page history and exact revision/item reads
verify record-era catalogues and signatures before disclosure. Both stores
expose an immutable bounded catalog-mutation log; PostgreSQL readiness verifies
the exact managed schema and catalog lineage. Application fixtures exercise the
real auth, tenancy, security,
...
bounded search and exact-read history kernel and both store implementations are
usable now;
...
| S2 recorder and memory | alpha | Manual event/entity append, grouping, idempotency, retry/reconciliation, concurrent memory persistence, signed public one-page history and exact revision/item reads are usable; protected disclosure belongs to S3 |
...
| S5 PostgreSQL | partial alpha | Exact schema readiness, catalog activation/readback, append, transaction joining, lookup, bounded search, exact revision/item inspection and restart identity are implemented; stable cursors and advanced control parity are owed |
...
| S3 advanced investigation/control | in progress | Exact inspection is shipped; attempts, cursors, reconstruction, comparison, neighbours, corrections, holds and purge planning remain planned |
