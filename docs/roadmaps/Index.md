...
| [2026-09-01-postgres-event-sourcing-roadmap.md](2026-09-01-postgres-event-sourcing-roadmap.md) | Current proposed PostgreSQL event-sourcing boundary, rebased 2026-09-06 on the landed service/storage chains, the `otel` module and the `tenancy` package: one `eventpg` module, direct aggregate/UoW API first, neutral delivery injection, no cross-extension bridges, and an E0 blocked only on a named aggregate |
