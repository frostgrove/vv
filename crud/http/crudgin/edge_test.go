...
func TestABodyPastTheCapIsRefusedAndReachesNoRepository(t *testing.T) {
	eng, f := mount(t, MaxBody[Widget, int64, WidgetUpdate](64))

	body := `{"name":"` + strings.Repeat("x", 512) + `"}`
	r := do(t, eng, http.MethodPost, "/widgets", body)
	...
	small := `{"name":"ok"}`
	if r := do(t, eng, http.MethodPost, "/widgets", small); r.status != http.StatusCreated {
...
func TestTheDefaultCapAcceptsAnOrdinaryBody(t *testing.T) {
	...
	if r := do(t, eng, http.MethodPost, "/widgets", body); r.status != http.StatusCreated {
...
func TestAnUnencodableResponseIsAServerFaultThatSaysNothing(t *testing.T) {
	eng, _ := mount(t, WithTransform[Widget, int64, WidgetUpdate](func(*gin.Context, Widget) any { return make(chan int) }))

	r := do(t, eng, http.MethodGet, "/widgets/1", "")
	...
	eng2, _ := mount(t, WithTransform[Widget, int64, WidgetUpdate](func(_ *gin.Context, w Widget) any { return w.Name }))
	if r := do(t, eng2, http.MethodGet, "/widgets/1", ""); r.status != http.StatusOK {
