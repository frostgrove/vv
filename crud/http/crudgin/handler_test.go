...
// "count". Gin's router gives a static segment priority over a parameter one,
// so unlike Fiber this does not depend on registration order — but the
// guarantee a caller gets is the same one, and it is worth pinning.
