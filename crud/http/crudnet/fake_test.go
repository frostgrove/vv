...
func mount(t *testing.T, options ...Option[Widget, int64, WidgetUpdate]) (*http.ServeMux, *fakeRepo) {
	t.Helper()
	f := newFake()
	mux := http.NewServeMux()
	New[Widget, int64, WidgetUpdate](f, options...).Mount(mux, "/widgets")
	return mux, f
}
...
func mountHandler[In, P, R any](h *ResourceFor[Widget, int64, WidgetUpdate, In, P, R]) *http.ServeMux {
	mux := http.NewServeMux()
	h.Mount(mux, "/widgets")
	return mux
}
