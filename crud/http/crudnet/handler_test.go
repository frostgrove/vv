...
// The literal path wins over /{id} instead of being read as an entity called
// "count". ServeMux gives the more specific pattern precedence, so unlike Fiber
// this does not depend on registration order — but the guarantee a caller gets
// is the same one, and it is worth pinning.
...
// Mount goes onto a mux that already exists, under any prefix — the other half
// of the documented set-up, and the one a real application with a versioned
// path uses.
