...
func TestEveryGeneratedRouteBubblesToTheProcessRendererWithItsHops(t *testing.T) {
	for _, tc := range []struct{ name, method, target, body string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			inner := mountHandler(resource)
			app := http.NewServeMux()
			app.Handle("/", Errors(crudhttp.WithCodes(installedCodes(t)))(inner))
...
func TestAManuallyRegisteredGeneratedMethodInstallsItsHops(t *testing.T) {
	...
	resource := ServingFor(Service[Widget, int64, WidgetUpdate](service), widgetMapper{})
	app := http.NewServeMux()
	app.HandleFunc("POST /manual", resource.Create)
...
func TestABodyPastTheCapIsRefusedAndReachesNoRepository(t *testing.T) {
	mux, f := mount(t, MaxBody[Widget, int64, WidgetUpdate](64))

	body := `{"name":"` + strings.Repeat("x", 512) + `"}`
	r := do(t, mux, http.MethodPost, "/widgets", body)
	...
	small := `{"name":"ok"}`
	if r := do(t, mux, http.MethodPost, "/widgets", small); r.status != http.StatusCreated {
...
func TestTheDefaultCapAcceptsAnOrdinaryBody(t *testing.T) {
	...
	if r := do(t, mux, http.MethodPost, "/widgets", body); r.status != http.StatusCreated {
...
func TestAnUnencodableResponseIsAServerFaultThatSaysNothing(t *testing.T) {
	mux, _ := mount(t, WithTransform[Widget, int64, WidgetUpdate](func(*http.Request, Widget) any { return make(chan int) }))

	r := do(t, mux, http.MethodGet, "/widgets/1", "")
	...
	mux2, _ := mount(t, WithTransform[Widget, int64, WidgetUpdate](func(_ *http.Request, w Widget) any { return w.Name }))
	if r := do(t, mux2, http.MethodGet, "/widgets/1", ""); r.status != http.StatusOK {
