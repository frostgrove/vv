...
func present(_ fiber.Ctx, w Widget) any {
...
func TestBeforeSaveMutationReachesTheRepository(t *testing.T) {
	stamp := BeforeSave[Widget, int64, WidgetUpdate](func(c fiber.Ctx, w *Widget) error {
		w.OwnerID = 99
		return nil
	})
...
func TestBeforeSaveCanRefuseTheRequest(t *testing.T) {
	app, fake := mount(t, BeforeSave[Widget, int64, WidgetUpdate](func(c fiber.Ctx, w *Widget) error {
...
func TestWithScopeNarrowsEveryRead(t *testing.T) {
	tenant := WithScope[Widget, int64, WidgetUpdate](func(c fiber.Ctx) ([]crud.Option, error) {
...
func TestWithScopeIsANDedWithTheClientFilter(t *testing.T) {
	app, fake := mount(t, WithScope[Widget, int64, WidgetUpdate](func(c fiber.Ctx) ([]crud.Option, error) {
		return []crud.Option{crud.Where(crud.Eq("OwnerID", int64(7)))}, nil
	}))
...
func TestWithErrorHandlerReplacesTheMapping(t *testing.T) {
	app, fake := mount(t, WithErrorHandler[Widget, int64, WidgetUpdate](func(c fiber.Ctx, err error) error {
		if errors.Is(err, crud.ErrNotFound) {
			return c.Status(http.StatusGone).JSON(fiber.Map{"gone": true})
		}
		return c.Status(http.StatusTeapot).JSON(fiber.Map{"gone": false})
	}))
...
func TestNewInfersItsTypeParametersFromTheRepository(t *testing.T) {
	fake := newFake()
	app := fiber.New()
	app.Use("/widgets", New(fake).Routes())
...
func TestTheHookStillRunsAfterTheServerOwnedFieldsAreCleared(t *testing.T) {
	var seen Widget
	hook := BeforeSave[Widget, int64, WidgetUpdate](func(_ fiber.Ctx, w *Widget) error {
		seen = *w
		return nil
	})
...
// localeRequest sends one create through the mounted app with the header under
