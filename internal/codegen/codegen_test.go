...
func TestAnExplicitKeyWinsOverTheWellKnownGormIDConvention(t *testing.T) {
	source := `package gormstore
...
`
	...
	assertAssignedGeneratedKey(t, out, "Team", "Key")
	input := decl(t, out, "type TeamInput struct {")
	if !declares(input, "ID") {
		t.Fatalf("gorm.Model's conventional ID stole the explicit Key declaration:\n%s", out)
	}
}
...
`,
		...
		{
			name: "trimmed effective database column",
			source: `package store
...
func TestAnAssignedPrimaryKeyRemainsInTheGeneratedInput(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "string", declaration: `Key string @db:"key,pk"@`},
		{name: "named UUID", prefix: "type UUID [16]byte\n\n", declaration: `Key UUID @db:"key,pk"@`},
		{name: "uintptr is not an auto integer", declaration: `Key uintptr @db:"key,pk"@`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			source := "package m\n\n" + tc.prefix + "type Document struct {\n\t" + tc.declaration + "\n\tTitle string `db:\"title\"`\n}\n"
			out := gen(t, map[string]string{"model.go": source}, withAdapter)
			assertAssignedGeneratedKey(t, out, "Document", "Key")
		})
	}
}
...
func TestIntegralPrimaryKeysFollowTheRuntimeAutoConvention(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "explicit primary key", key: `Key int64 @db:"key,pk"@`},
		{name: "conventional ID field", key: `ID int64`},
		{name: "named integral ID", prefix: "type Snowflake int64\n\n", key: `ID Snowflake`},
		{name: "runtime ID column lookup", key: `Key int16 @db:"ID"@`},
		{name: "conventional id column", key: `Key int32 @db:" id"@`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			source := "package m\n\n" + tc.prefix + "type Document struct {\n\t" + tc.key + "\n\tTitle string `db:\"title\"`\n}\n"
			out := gen(t, map[string]string{"model.go": source}, withAdapter)
			key := strings.Fields(tc.key)[0]
			assertAutoGeneratedKey(t, out, "Document", key)
			if response, err := runGenerated(t, tags(source), out); err != nil {
				t.Fatalf("generated package and runtime metadata disagree: %v\n%s\n---- generated ----\n%s", err, response, out)
			}
		})
	}
}

func TestNoAutoKeepsAnIntegralPrimaryKeyAssigned(t *testing.T) {
	for _, tc := range []struct {
		name string
		key  string
	}{
		{name: "explicit primary key", key: `Key int64 @db:"key,pk,noauto"@`},
		{name: "conventional ID field", key: `ID int64 @db:",noauto"@`},
		{name: "conventional id column", key: `Key int64 @db:" id,noauto"@`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertAssignedGeneratedKey(t, out, "Document", key)
			if response, err := runGenerated(t, tags(source), out); err != nil {
				t.Fatalf("noauto generated package refused to start: %v\n%s\n---- generated ----\n%s", err, response, out)
			}
		})
	}
}

func TestAnExplicitAssignedKeyWinsOverAnOrdinaryIDColumn(t *testing.T) {
	source := `package m
...
`
	...
	if !declares(input, "ID") || !strings.Contains(paths, `"ID":`) {
		t.Fatalf("the ID convention stole a model's explicitly declared key:\n%s", out)
	}
	if response, err := runGenerated(t, tags(source), out); err != nil {
		t.Fatalf("explicit-key package refused to start: %v\n%s\n---- generated ----\n%s", err, response, out)
	}
}

func TestExplicitAutoWinsOverNoAutoIndependentOfTagOrder(t *testing.T) {
	for _, options := range []string{"auto,noauto", "noauto,auto"} {
		t.Run(options, func(t *testing.T) {
			source := "package m\n\ntype Document struct {\n\tID int64 `db:\"id,pk," + options + "\"`\n\tTitle string `db:\"title\"`\n}\n"
			out := gen(t, map[string]string{"model.go": source}, withAdapter)
			assertAutoGeneratedKey(t, out, "Document", "ID")
		})
	}
}

func TestExplicitAutoCanMarkANonIntegralDatabaseOwnedKey(t *testing.T) {
	source := `package m

type Document struct {
	Key   string @db:"key,pk,auto"@
	Title string @db:"title"@
}
`
	out := gen(t, map[string]string{"model.go": source}, withAdapter)
	assertAutoGeneratedKey(t, out, "Document", "Key")
	if response, err := runGenerated(t, tags(source), out); err != nil {
		t.Fatalf("explicit non-integral auto package refused to start: %v\n%s\n---- generated ----\n%s", err, response, out)
	}
}

func TestDatabaseOwnedOnlyModelGeneratesAnEmptyTotalAdapter(t *testing.T) {
	source := `package m
...
`
	...
	for _, want := range []string{
		...
		`var SequencePaths = port.MustPathMap[Sequence](port.PathMap{}, "ID")`,
	} {
		if !strings.Contains(out, want) {
			t.Fatalf("database-owned-only adapter lacks %q:\n%s", want, out)
		}
	}
	if strings.Contains(out, "\tout.ID = in.ID") {
		t.Fatalf("empty mapper copied the database-owned key:\n%s", out)
	}
	if response, err := runGenerated(t, tags(source), out); err != nil {
		t.Fatalf("empty total adapter refused to start: %v\n%s\n---- generated ----\n%s", err, response, out)
	}
}

func TestPrimaryKeyConventionIgnoresDeclarationsRuntimeDoesNotIndex(t *testing.T) {
	for _, tc := range []struct {
		name  string
		field field
	}{
		{name: `db:"-"`, field: field{Name: "ID", Tag: "-", Skip: true, Integral: true}},
		{name: "relation", field: field{Name: "ID", HasRel: true, Rel: "belongs_to"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			m := &model{Name: "Document", Fields: []field{tc.field, {Name: "Title", Type: "string"}}}
			if problems := resolvePrimaryKey(m); len(problems) != 0 {
				t.Fatalf("resolving a convention-only model: %v", problems)
			}
			if key, ok := m.pk(); ok {
				t.Fatalf("non-column %s became primary key: %+v", tc.field.Name, key)
			}
		})
	}
}

func TestMultipleExplicitPrimaryKeysFailDeterministically(t *testing.T) {
	err := genError(t, `package m

type Document struct {
	Second int64 @db:"second,pk"@
	First  int64 @db:"first,pk"@
}
`)
	if err == nil || !strings.Contains(err.Error(), "multiple primary keys First, Second") {
		t.Fatalf("multiple explicit keys did not fail with stable field names: %v", err)
	}
}

func assertAutoGeneratedKey(t *testing.T, out, model, key string) {
	t.Helper()
	input := decl(t, out, "type "+model+"Input struct {")
	paths := decl(t, out, "var "+model+"Paths = port.MustPathMap["+model+"](port.PathMap{")
	if declares(input, key) || strings.Contains(paths, `"`+key+`":`) || strings.Contains(out, "\tout."+key+" = in."+key) {
		t.Fatalf("auto-generated key %s leaked into the input, mapper or path map:\n%s", key, out)
	}
	if !strings.Contains(paths, `"`+key+`"`) {
		t.Fatalf("auto-generated key %s is not declared as a path-map exclusion:\n%s", key, paths)
	}
}

func assertAssignedGeneratedKey(t *testing.T, out, model, key string) {
	t.Helper()
	input := decl(t, out, "type "+model+"Input struct {")
	paths := decl(t, out, "var "+model+"Paths = port.MustPathMap["+model+"](port.PathMap{")
	if !declares(input, key) || !strings.Contains(paths, `"`+key+`":`) || !strings.Contains(out, "\tout."+key+" = in."+key) {
		t.Fatalf("assigned key %s disappeared from the input, mapper or path map:\n%s", key, out)
	}
}
