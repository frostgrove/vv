...
func New(spec Spec) (*Authority, error) {
	...
	if !admission.valid() {
		return nil, errors.New("tenancy: this admission names a class or a lifecycle state that does not exist, " +
			"or names a class with no states at all; a policy that cannot be read is not narrowed to nothing, it is refused")
	}
