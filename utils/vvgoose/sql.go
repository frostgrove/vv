...
`, table, table)), nil
	...
	for _, field := range model.Fields {
		...
		seen[folded] = true
		definition, err := renderColumn(engine, field)
		if err != nil {
...
func renderColumn(engine vvdb.Engine, field modelscan.Field) (string, error) {
	name := quoteIdentifier(engine, field.Column)
	if name == "" {
		return "", fmt.Errorf("column %q is not a valid identifier", field.Column)
	}
	typ, integer := sqlType(engine, field.GoType)
	if typ == "" {
	...
	if field.PrimaryKey && field.Auto && integer {
		switch engine {
		case vvdb.Postgres:
	...
	} else if field.Generated && timestampType(field.GoType) {
